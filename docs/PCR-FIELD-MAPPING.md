...
| ~~`overallConvictionDate`~~ | `court_appearance.overall_conviction_date` | — | **Removed.** No hearing-level source in CP — `OutboundPrisonCourtRegister`'s `Hearing`/`Offence` models confirm conviction date only exists per-offence (`Offence.convictionDate`), matching `PcrResult.offences[].convictionDate` |
