# PCR field mapping for `api-cp-crime-results-pcr`

**Status:** Draft. The API itself is CP-native (`openapi-spec.yml`) and is not scoped to any
single consumer — this document tracks, per CP-native field, where the value actually comes
from in CP. §1's table additionally cross-references one confirmed consumer's (HMPPS) own
Remand and Sentencing (RaS) physical data model (Court Case / Court Appearance / Charge /
Sentence / Period Length), kept here as reference material for that integration specifically —
**it does not define the contract's shape**. A field absent from RaS, or a mapping HMPPS doesn't
need, is not by itself a reason to exclude something another consumer needs; conversely, this
doc's own §6 records fields evaluated and left out because *no* consumer (HMPPS or otherwise) had
a confirmed need, which is a different, contract-wide judgement, not an HMPPS-specific one. It
...
### Judicial Result (`ResultText`, formerly `JudicialResult`; item prompts are `Text`, formerly `JudicialResultPrompt`)

**2026-08-14 — simplified, breaking change.** `financial`, `category`, `convicted`,
...
| ~~`imprisonmentPeriod`~~ / ~~`totalCustodialPeriod`~~ | `period_length.*` | `imprisonmentPeriod`/`totalCustodialPeriod` (free-text string, e.g. "6 Months 1 week") | Both **removed 2026-08-14** — no confirmed consumer need |
...
- **Defendant identity — name/DOB/address: carried, encrypted at rest; gender/nationality/aliases: still absent.** `title`/`firstName`/`middleName`/`lastName`/`dateOfBirth`/`address` are present on the `Defendant`/`Address` schemas — a confirmed requirement now needs this data through the API; see the service repo's `docs/pipeline/adrs/002-carry-defendant-pii-encrypted-at-rest.md` for the decision and the encryption mechanism. This reverses this document's own earlier position (defendant identity as HMPPs-irrelevant because their `court_case` table has no name/DOB/address columns) — that reasoning was scoped to one consumer's needs, and doesn't hold once another confirmed need exists. `gender`/`nationality`/`aliases` remain genuinely absent — no field exists for them and no reversal applies; revisit only if a real requirement surfaces for those specifically.
...
`recordedAt`). `versionId` is optional, not required, and there is no `latest` special value: omitted, it lists
every version's metadata; supplied, it filters the same list down to the one matching entry — `recordedAt` is what
lets a caller determine "latest" themselves while `id` stays nullable in phase 1. The previous open question about
whether HMPPS calls a defendant-scoped endpoint or filters a case-level response no longer applies. Note: there is
...
- **Decision recorded — candidate fields evaluated and NOT added to the CP-native contract, for lack of any confirmed consumer need:** prosecution/defence counsel, attending solicitor name, `defendantResults[]`/`caseResults[]` (already closed per §2a), `gender`/`nationality`, `aliases`, `ljaName`/`courtHouseAddress`, `allocationDecision`/`indicatedPleaValue`. All are confirmed to exist in CP's real PCR payload (cross-validated between the Function App's outbound mapper and Progression's `PrisonCourtRegisterPdfPayloadGeneratorTest.java` fixtures), but none has an identified requirement from any consumer. (`name`/`dateOfBirth`/`address` were originally grouped with this list on the same reasoning — no confirmed need — but that's since reversed: see §2c and the service repo's ADR-002. `aliases` was initially bundled alongside name/DOB/address without independent justification and stays excluded here, not swept up in that reversal.) Revisit only if a real requirement surfaces; do not add speculatively.
